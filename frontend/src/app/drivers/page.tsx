...
export default function Drivers() {
  ...
  const fetchDrivers = async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data = await res.json();
        // The API returns users list
        const rawUsers = data.users || [];
        // Normalize role mapping logic
        const normalized = rawUsers.map((u: any) => ({
          id: u.id,
          name: u.name,
          phone: u.phone,
          email: u.email,
          region: u.region || "Casablanca",
          created_at: u.joinedDate || new Date().toISOString(),
          email_verified_at: u.verified ? new Date().toISOString() : null,
          verified: u.verified
        }));
  ...
  return (
    <div className="p-8 space-y-8 max-w-7xl mx-auto">
          ...
          <>
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between">
              <div>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <h3 className="text-2xl font-black text-gray-900">{drivers.filter(d => d.verified).length}</h3>
                </div>
              </div>
            ...
            <div className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center justify-between">
              <div>
                ...
                <h3 className="text-2xl font-black text-gray-900 mt-1">{drivers.filter(d => !d.verified).length}</h3>
              </div>
              ...
              <tbody className="divide-y divide-gray-50 text-xs font-semibold text-gray-800">
                {drivers.map((driver) => {
                  ...
                  return (
                    <tr 
                      key={driver.id} 
                      onClick={() => userRole === "merchant" && setSelectedDriver(driver)}
                      className={`transition-colors ${userRole === "merchant" ? "hover:bg-gray-50/40 cursor-pointer" : "hover:bg-gray-50/20"}`}
                    >
                        ...
                        <>
                          <td className="py-4 px-6">
                            <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold border ${
                              driver.verified ? "bg-emerald-50 text-emerald-700 border-emerald-100" : "bg-amber-50 text-amber-700 border-amber-100"
                            }`}>
                              {driver.verified ? "Approved" : "Pending Verification"}
                            </span>
                          </td>
      ...
      {/* Driver Performance profile drawer */}
